...
import com.eap.framework.base.pojo.AvatarResult;
import com.eap.framework.base.pojo.FileResult;
import com.eap.framework.base.pojo.MarkDownResult;
import com.eap.framework.base.pojo.Result;
import com.eap.framework.base.service.UploaderService;
import com.eap.framework.util.SecurityUtil;
import com.eap.framework.utils.DateUtil;
import com.eap.framework.utils.FileUtil;
...
public class UploaderController {
        ...
        if (contentType.indexOf("multipart/form-data") >= 0) {
            ...
            for (Iterator<Map.Entry<String, MultipartFile>> it = fileMap.entrySet().iterator(); it.hasNext(); avatarNumber++) {
                ...
                if (fieldName.equals("__initParams")) {
                    ...
                    inputStream = new BufferedInputStream(mFile.getInputStream());
                    outputStream = new BufferedOutputStream(new FileOutputStream(virtualPath.replace("/",File.separator)));
     ...
     * 跳转到数据导入上传界面
     */
    @RequestMapping(value="/dataUploader",method = RequestMethod.GET)
    public String dataUploader(String config,HttpServletRequest request){
        request.setAttribute("config",config);
        return "base/file/data_uploader";
    }
    ...
    public String imageUploader(String config,HttpServletRequest request){
        request.setAttribute("config",config);
        return "base/file/image_uploader";
    }
            ...
            if (!file.isEmpty()) {
                ...
                try {
                    ...
                    /*preview.add("<div class=\"file-preview-other\">\n" +
                            ...
                            "</div>");*/
                     msg.setFilePath(sysFile.getFilePath());
