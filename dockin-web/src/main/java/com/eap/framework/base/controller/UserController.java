...
import com.eap.framework.base.entity.UserAvatar;
...
public class UserController {
    ...
    private String changePwd(HttpServletRequest request) {
        request.setAttribute("curUser", SecurityUtil.getUser());
        return "base/user/user_changePwd";
    }

    @RequestMapping(method = RequestMethod.POST, value = "/password/change")
    @ResponseBody
    private Result passwordChanged(String userId, String loginName, String oldPwd, String newPwd) {
        //判断修改账户是否成功
        String hql = "from User where id!=:userId and loginName=:loginName";
        Map<String, Object> param = new HashMap<>();
        param.put("userId", userId);
        param.put("loginName", loginName);
        List<User> userList = userService.find(hql, param);
        if (!userList.isEmpty()) {
          return new Result(false,"当前账户已经存在，请选择另一个账户");
        }
        User user=this.getUser(userId);
        String pwd=EncryptUtil.getPassword(oldPwd, user.getLoginName());
        if(!pwd.equals(user.getPassword())){
            return new Result(false,"原密码错误");
        }
        //设置新密码
        pwd=EncryptUtil.getPassword(newPwd,loginName);
