package com.eap.framework.query.controller;
...
public class QueryController {
    ...
    public Map<String, Object> loadData(String reqObj) throws Exception {
        ...
        return queryService.loadData(reqObj,true);
    }
    ...
    @ResponseBody
    public Result exportData(String reqObjs, String tableName, HttpServletResponse response)
            throws Exception {
        String tempFile = queryService.exportData(reqObjs, tableName);
        //response.getWriter().print(tempFile);
        return new Result(true,tempFile,"导出成功");
    }

    /**
     * 导出模板数据
     */
    @RequestMapping(value="/exportDataByTemplate")
    @ResponseBody
    public Result exportDataByTemplate(String reqParam, String tableName,String template, HttpServletResponse response)
            throws Exception {
        Result result = queryService.exportDataByTemplate(reqParam, tableName,template);
        return result;
    }
    ...
    public void downExport(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            File file = new File(ExportUtil.getFilePath(templateName));
    ...
    public List<TreeNode> getColumns(String queryId, HttpSession session) {
        ...
        for (Column column : query.getColumnList()) {
            ...
            node.setId(StrUtil.isNotBlank(column.getId())?column.getId():column.getKey());
