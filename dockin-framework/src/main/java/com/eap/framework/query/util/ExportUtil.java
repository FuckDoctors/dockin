...
public class ExportUtil {
    ...
    private static WritableCellFormat getLeftFormat() {
        WritableFont FONT_BODY = new WritableFont(WritableFont.TIMES, 12, WritableFont.NO_BOLD, false);
    ...
    private static WritableCellFormat getRightFormat() {
        WritableFont FONT_BODY = new WritableFont(WritableFont.TIMES, 12, WritableFont.NO_BOLD, false);
        WritableCellFormat right = new WritableCellFormat(FONT_BODY);
        right = getCellFormat(right, Alignment.RIGHT);
        try {
            right.setWrap(true);
        } catch (WriteException ex) {
            return right;
     ...
     * 是否需要导出
     */
    private static boolean isExport(Column column,List<String> ignoreColumns) {
        //隐藏列不导出
        if(!ignoreColumns.isEmpty()){
            for (String colName : ignoreColumns) {
               if(colName.equals(column.getId())||colName.equals(column.getKey())){
                   return false;
               }
            }
        }
        if(column.getIsExport())
            return true;
        if (column.getIsServerCondition() || column.getHidden()||!column.getIsExport())
            return false;
        return true;
    }
     ...
     */
    public static void writeTitle(QueryCondition queryCondition, Query query, WritableSheet sheet,List<String> ignoreColumns) throws QueryException {
        try {
            //写标题
            Label title = new Label(0, 0, StrUtil.isEmpty(queryCondition.getSheetTitle()) ?
                    queryCondition.getSheetName() : queryCondition.getSheetTitle(), getTitleFormat());
            //合并标题单元格
            int lastCol = -1;
            List<Column> colList = query.getColumnList();
            for (Column column : colList) {
                if (ExportUtil.isExport(column,ignoreColumns))
                    lastCol++;
            }
            if (lastCol == -1) {
            ...
            if (!StrUtil.isEmpty(queryCondition.getSheetSubTitle())) {
                Label subTitle = new Label(0, 1, queryCondition.getSheetSubTitle(), getLeftFormat());
     ...
     * @throws QueryException 执行异常
     */
    public static void writeHeader(QueryCondition queryCondition, Query query, WritableSheet sheet,List<String> ignoreColumns) throws QueryException {
        ...
        try {
            ...
            for (int h = 0; h < colList.size(); ++h) {
                ...
                if (isExport(column,ignoreColumns)) {
                    // 创建表头列
                    head = new Label(h - ignoreCount, headIndex, column.getHeader(), getHeaderFormat());
                    sheet.addCell(head);
     ...
     * @param  ignoreColumns 自定义列中不需要导出的列
     * @throws QueryException 执行异常
     */
    public static void writeData(QueryCondition queryCondition, Query query, WritableSheet sheet, List objList, Class objClass,List<String> ignoreColumns) throws QueryException {
        int headIndex = getTitleIndex(queryCondition);
        try {
            ...
            for (int rowIndex = 1; rowIndex <= rowLen; ++rowIndex) {
                ...
                for (int colIndex = 0; colIndex < colLen; ++colIndex) {
                    ...
                    if ("rowIndex".equals(column.getKey())) {
                        // 设置当前列序号
                        Number num = new Number(0, rowIndex + headIndex, rowIndex, getIntegerFormat());
                        sheet.addCell(num);
                        continue;
                    }
                    ...
                    if (isExport(column,ignoreColumns)) {
                        ...
                        if (column.getClassType().equals("java.lang.Double") &&StrUtil.isEmpty(column.getSuffix())) {
                            if (!StrUtil.isEmpty(data)) {
                                ...
                                Label label = new Label(colIndex - ignoreCount, rowIndex + headIndex, "", getDecimalFormat());
                                sheet.addCell(label);
                            }
            ...
            if (objClass != null) {
                ...
                if (Date.class.getName().equals(column.getClassType())
                    ...
                    data = column.getDateFormat() != null ? DateUtil.format(
                            (Date) dataMap.get(column.getKey()), DateUtil.getDateFormatter(column.getDateFormat())) : DateUtil
                            .format((Date) dataMap.get(column.getKey()));
                } else {
                    if (dataMap.get(column.getKey()) != null) {
                        data = dataMap.get(column.getKey()).toString();
                    }
                }
            }
            return data;
    ...
    private static WritableCellFormat getAlignFormat(Column column) {
        ...
        if (column.getAlign().equals("right")) {
            format = getRightFormat();
        } else if (column.getAlign().equals("left")) {
            format = getLeftFormat();
        } else {
            format = getCenterFormat();
        }
        return format;
    }
    ...
    public static String getFilePath(String fileName){
        File file=new File(PropertiesUtil.getValue("templatePath"));
        if(!file.exists()){
            file.mkdir();
        }
     ...
     * 获取模板文件路径
     */
    public static String getTemplatePath(String fileName, HttpServletRequest request){
        String dir=request.getRealPath("/");
        String path= PropertiesUtil.getValue("xlsPath");
